"""1D-CNN classifier: treats the feature vector as a 1D sequence and slides convolutional filters across it."""
...
def train_cnn1d(X_train, y_train, X_val, y_val, *, cfg: dict, seed: int,
                n_classes: int) -> CNN1D:
    ...
    model = CNN1D(Xt.shape[1], n_classes, p["dropout"]).to(device)
    ...
    for epoch in range(1, p["epochs"] + 1):
        ...
        log.info(f"CNN1D epoch {epoch:02d}  loss={total_loss / len(Xt):.4f}  val_macroF1={val_f1:.4f}")
...
def cnn1d_predict(model: CNN1D, X) -> np.ndarray:
...
def cnn1d_predict_proba(model: CNN1D, X) -> np.ndarray:
    device = next(model.parameters()).device
    model.eval()
    with torch.no_grad():
        xb = torch.from_numpy(X.astype(np.float32)).to(device)
